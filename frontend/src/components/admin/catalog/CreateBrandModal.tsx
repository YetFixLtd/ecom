...
import { createBrand, type CreateBrandData } from "@/lib/apis/brands";
...
export function CreateBrandModal({ onClose, onSuccess }: Props) {
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    try {
      ...
      await createBrand(token, values as CreateBrandData);
      onSuccess();
    } catch (error) {
      if (error instanceof AxiosError && error.response) {
        ...
        setServerError(data.message || "Failed to create brand.");
      } else {
