...
export default function EditWarehousePage() {
    ...
    const fetchWarehouse = async () => {
      try {
        ...
        reset({
          ...
          address1: warehouseData.address1 || "",
          address2: warehouseData.address2 || "",
          city: warehouseData.city || "",
          state_region: warehouseData.state_region || "",
          postal_code: warehouseData.postal_code || "",
          country_code: warehouseData.country_code || "",
          is_default: warehouseData.is_default,
        });
      } catch (error) {
        if (error instanceof AxiosError) {
          setServerError(
            error.response?.data?.message || "Failed to load warehouse"
          );
        } else {
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    try {
      const data: UpdateWarehouseData = {};
      if (values.name !== undefined) data.name = values.name;
      if (values.code !== undefined) data.code = values.code;
      data.address1 = values.address1 || null;
      data.address2 = values.address2 || null;
      data.city = values.city || null;
      data.state_region = values.state_region || null;
      data.postal_code = values.postal_code || null;
      data.country_code = values.country_code || null;
      if (values.is_default !== undefined) data.is_default = values.is_default;
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-12">
        <div className="text-sm text-gray-500">Loading warehouse...</div>
      </div>
    );
  }
